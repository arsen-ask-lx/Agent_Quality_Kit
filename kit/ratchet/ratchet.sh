...
#
# ПОСТОЯННАЯ ЗАПИСЬ — `~ <ключ>`. Пропускается, как весь долг, но по ОТСУТСТВИЮ не вычёркивается:
# есть находки, которые выпадают не каждый прогон (гонка потоков, чужой код, сеть). audit_project
# 2026-09-22: запись N+1 в чужой библиотеке токенов зажигал один тест при гонке двух потоков, и
# храповик требовал «вычеркни». Этот вычёркивал сам — и в следующий раз та же находка приходила
# НОВОЙ: красный конвейер без единой правки кода. Причина обязательна — строкой выше
# `# постоянная: <почему>` (или `# permanent: <why>`): метка без неё стала бы способом заморозить
# любой долг навсегда и молча. Просто комментарий не годится — шапка реестра тоже комментарий.
NOREASON="$(awk '/^~ /{ if (prev !~ /^[[:space:]]*#[[:space:]]*(постоянная|permanent):[[:space:]]*[^[:space:]]/) print NR": "$0 } { prev = $0 }' "$REG")"
if [ -n "$NOREASON" ]; then
  echo "постоянная запись без причины — метка «~» не действует:"
  printf '%s\n' "$NOREASON" | sed 's/^/  /'
  echo "  почини: строкой выше напиши «# постоянная: <почему находка выпадает не каждый прогон>»."
  exit 2
fi
WAS="$(grep -vE '^\s*(#|$)' "$REG" | sed 's/^~ //' | LC_ALL=C sort -u)"
PERM="$(grep -E '^~ ' "$REG" | sed 's/^~ //' | LC_ALL=C sort -u)"

NEW="$(comm -23 <(printf '%s\n' "$NOW") <(printf '%s\n' "$WAS"))"
GONE="$(comm -23 <(comm -13 <(printf '%s\n' "$NOW") <(printf '%s\n' "$WAS")) <(printf '%s\n' "$PERM") | grep -v '^$' || true)"
...
if [ -n "$GONE" ]; then
  ...
  #
  # Вычёркиваем построчно по месту, а не собираем файл заново: постоянная запись и её причина
  # обязаны остаться рядом, а шапка — наверху.
  awk 'NR == FNR { gone[$0] = 1; next }
       /^[[:space:]]*(#|$)/ || /^~ / { print; next }
       !($0 in gone) { print }' <(printf '%s\n' "$GONE") "$REG" > "$REG.tmp"
  mv "$REG.tmp" "$REG"
