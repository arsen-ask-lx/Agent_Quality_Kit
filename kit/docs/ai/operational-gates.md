...
### Детектор во всех тестах, а не бюджет в избранных

Бюджет запросов ставят на те ручки, про которые вспомнили. Надёжнее детектор ленивой загрузки
фреймворка (Django — `django-zeal`, Rails — `bullet`/`strict_loading`, Laravel —
`Model::preventLazyLoading()`), подключённый ко ВСЕМ тестам. Устройство, проверенное в
audit_project 2026-09-22 (три медленных экрана в проде, которые правило «у каждого списка тест
бюджета» пропустило, потому что жило только словами):

1. **окно — один HTTP-запрос, а не весь тест**: выборка сессии и пользователя законно
   повторяется в каждом запросе теста, и окно «тест» даёт ложные тревоги;
2. **нарушение копится и роняет тест после**, а не бросается в запросе: широкий `except` во
   вьюхе проглотил бы исключение, и гейт промолчал бы ровно там, где нужен;
3. **реестр долга** (маршрут, модель, поле, причина, задача), проверяемый на старте: модель и
   поле существуют, причина и владелец не пусты;
4. **храповик только на полном прогоне** и только для записей с владельцем: находка в чужом коде,
   которую зажигает гонка потоков, выпадает не каждый прогон, и «вычеркни» по ней — случайный
   красный конвейер (у храповика AQK для этого есть постоянная запись `~`);
5. «это полный прогон» — **один флаг на все гейты о наборе целиком**, выставленный ровно в одном
   задании конвейера, которое не отбирает тесты.

### Журнал ручек: каждую ручку проверяет тест с данными

Раньше здесь стояла честная граница: двадцать один тест бюджета не доказывает двадцать вторую
ручку, а общий гейт возможен, только когда проект умеет машинно определить множество ручек.
audit_project 2026-09-22 это множество определил — и граница снята.

Детектор «на строку» и любая проверка договора ответа (арбитр OpenAPI) проходят на пустом ответе
и на одной строке: повторять нечего, а пустой список проходит любую схему. Замер там: из 179
GET-ручек 15 не вызывал ни один тест, ещё 24 — только с 0–1 строкой, и ровно так прятались все
три медленных экрана — тест был, с одной строкой. Устройство:

1. middleware ТЕСТОВЫХ настроек записывает каждый GET: маршрут (`resolver_match.route`), код,
   число строк ответа;
2. **перечень ручек — у фреймворка**, не своя копия (Django — обход `get_resolver()`, FastAPI —
   `app.routes`, Express — роутер, Rails — `Rails.application.routes`);
3. на полном прогоне каждая ручка обязана хоть раз ответить успехом, а список — хоть раз отдать
   2+ строки; долг — реестр, только укорачивается; на красном прогоне вердикт печатается
   справочно и итог не трогает.

Три ловушки, на которых прибор врал сам, — все найдены прогоном, ни одна чтением:

- **служебные алиасы фреймворка**: DRF-роутер даёт каждой ручке двойника `.format` и оглавление —
  первый перечень назвал 102 «не вызванных» ручки вместо 15. Сверять с независимым замером:
  расхождение в разы — сигнал, что врёт прибор;
- **конверт пагинатора** — словарь, чьи ключи ⊆ `{count, next, previous, results}`, а не «словарь,
  где есть `results`»: у карточки бывает своё поле `results`;
- **перечень на момент запроса**, а не на начало сессии: тест, включающий `DEBUG`, перестраивает
  адреса.

Цена: полный прогон 8:44 против 8:39 без журнала (одна машина, тихое окно). Побочный продукт —
список ручек, которых не вызывает никто: кандидаты на удаление. В audit_project среди них нашлась
карточка, отвечающая 200 с неверными данными (список — 1 слот, карточка той же компании — 0).

**Засев данных для общих обходчиков — минимум две строки с разными связями.** Одна запись на
модель в общем засеве ослепляет все ручки, которые он кормит; две записи с одной связью прячут
повтор за кэшем объекта.
