...
#
# NOTICE — НАЗВАТЬ, НЕ РОНЯЯ. `EXPECTED_VALUE_CHANGED`: ожидание переписано, а сила утверждения
# та же. Самая чистая форма подгонки выглядит именно так — «2+2 стало 5» при сломанном коде, — и
# до 2026-09-26 она проезжала молча: правила в списках не было, гейт возвращал ноль и не говорил
# ничего. Красить её нельзя, и это решение по замеру, а не осторожность. Корпус из 16 изменений
# (`research/2026-09-23-upstream-practices/corpus/test-changes/`, цена политики — `policy.py`):
#   как сейчас          — поймано 6 из 7, ложных обвинений 2 из 9;
#   с покраской правила — поймано 7 из 7, ложных обвинений 3 из 9.
# Третье ложное обвинение — поведение изменено ОСОЗНАННО, ожидание обновлено вместе с ним. По
# форме это то же самое: и там и там правится код и ожидание; отличие только в намерении, а
# намерение машине не видно. В живом проекте такая правка — самая частая, и гейт, который её
# блокирует, выключают целиком — тогда он не поймает и остальные шесть. Поэтому вердикт прежний,
# а строка появляется: молчание неотличимо от «чисто», и ровно это свод запрещает.
NOTICE='EXPECTED_VALUE_CHANGED'
#
...
CLASSED=$(printf '%s\n' "$JSON" | awk -v always="$ALWAYS" -v highonly="$HIGH_ONLY" -v notice="$NOTICE" '
  ...
  /"unit":/     {
    ...
    if (r != "") {
      ...
      noticed = !al && !blocking && r ~ ("^(" notice ")$")
      # Полка пишется первым полем, а не вторым проходом по тому же json: один разбор — один
      # источник правды о находках, и правило не может попасть в обе полки по рассогласованию.
      if (blocking) printf "B\t%s:%s: %s — %s [%s]\n", p, (u == "" ? "0" : u), r, m, s
      else if (noticed) printf "N\t%s:%s: %s — %s [%s]\n", p, (u == "" ? "0" : u), r, m, s
    }
    r = ""; s = ""; p = ""; u = ""; m = ""; al = 0
  }
')
...
# Названо и не уронено. Метка «почини:» здесь не ради вежливости: по ней прогон решает, показывать
# ли строку ЗЕЛЁНОГО гейта (`splitAdvice` в `tool/lib/scope.mjs`). Без метки сообщение печатается
# в никуда, и «посмотрел и нашёл» снова становится неотличимо от «посмотрел и чисто».
if [ -n "$NOTED" ]; then
  echo "  почини: ожидания тестов переписаны — убедись, что это решение, а не подгонка под код."
  printf '%s\n' "$NOTED" | sed 's/^/    /'
  echo "    вердикт не меняется: по форме осознанная смена поведения и подгонка одинаковы."
fi
exit 0
