...
- **Ослабление в стиле `node:assert` — дыра инструмента.** Проверено 2026-09-21 на
  `checkwash` 0.3.4: `assert.equal(sum(2, 2), 4)` → `assert.ok(sum(2, 2))` и
  `assert.strictEqual(…, 4)` → `assert.ok(…)` проходят (`verdict=pass`), тогда как та же подмена
  в jest (`toBe(4)` → `toBeTruthy()`) и в pytest (`assert x == 4` → `assert x`) даёт
  `ASSERT_WEAKENED`. Чинить это у них, а не обходом здесь; разбор —
  `research/competitors/code-quality-skill.md`.
