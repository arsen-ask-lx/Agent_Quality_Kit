...
# «ПУСТО» — НЕ «ЧИСТО». Гейт, который не нашёл ни одного файла своего типа, раньше молчал с кодом 0,
# ровно как на проверенном и чистом проекте. Замер 2026-09-26: так вели себя 10 из 13 сканирующих
# записей, а `.aqkignore`, скрывший весь код, делал немым гейт на проекте, где беда была. Код
# остаётся 0 (решение владельца: проект без таких файлов не должен краснеть всегда), но строка
# «пусто: …» печатается, и прогон показывает её под галочкой.
#
# Отбор тот же, что у гейта: обход с пропусками, свои образцы и .aqkignore отфильтрованы, дальше
# шаблон включения и, если нужно, исключения. Иначе «пусто» говорилось бы не тогда, когда гейт
# ничего не прочитал, а когда ничего не нашёл помощник.
#   nothing_to_check КАТАЛОГ ШАБЛОН_ВКЛЮЧЕНИЯ ЧТО_ИСКАЛИ [ШАБЛОН_ИСКЛЮЧЕНИЯ]
# Возвращает 0 (и печатает строку), если читать нечего; 1 — если есть что.
nothing_to_check() {
  _NT_DIR="$1"; _NT_INC="$2"; _NT_WHAT="$3"; _NT_EXC="${4:-}"
  _NT_ONE=$(find "$_NT_DIR" $(skip_find "$_NT_DIR") -type f -print 2>/dev/null \
    | grep -E "$_NT_INC" \
    | { if [ -n "$_NT_EXC" ]; then grep -vE "$_NT_EXC"; else cat; fi; } \
    | own_samples_filter "$_NT_DIR" | head -1)
  [ -n "$_NT_ONE" ] && return 1
  if [ -f "$_NT_DIR/.aqkignore" ]; then
    echo "пусто: $_NT_WHAT не нашлось — проверять было нечего; часть путей скрыта .aqkignore, проверь, не всё ли"
  else
    echo "пусто: $_NT_WHAT не нашлось — проверять было нечего"
  fi
  return 0
}
CODE_RE="\\.($(printf '%s' "$CODE_EXT" | tr ' ' '|'))\$"
TESTS_RE='(^|/)(test|tests|spec|__tests__)/|(^|/)test_[^/]*$|_test\.[^/]*$|\.(test|spec)\.[^/]*$'
