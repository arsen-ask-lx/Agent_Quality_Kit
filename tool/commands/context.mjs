...
function contextBlock(state, T = L.context) {
  ...
  // СОСТОЯНИЙ У ПРОБЫ ПЯТЬ, А ВЕТОК БЫЛО ТРИ. В «иначе» падали `off` (пробу выключили в
  // манифесте) и `unknown` (счётчик коммитов не взять либо непонятое значение поля) — и оба
  // печатались как «в местах пробы всё прикрыто». То есть выключенная и несостоявшаяся проверка
  // выдавались за проверку, сказавшую «чисто». Блок читает машина: она примет это за
  // разрешение. Перечисление исчерпывающее намеренно — новое состояние обязано сюда попасть,
  // а не утечь в «иначе».
