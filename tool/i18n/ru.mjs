...
export const ru = {
  ...
  doctor: {
    ...
    minBad: (v, top) => `--min ждёт номер ступени от 0 до ${top}: «${v}» не подходит. Порог, который ничего не держит, хуже отсутствующего, поэтому стоп до прогона.`,
    ...
    coversDisabled: (e, g, codes) => `заявка «${g} держит ${e}» неверна: правило ${codes} в конфиге линтера стоит в ВЫКЛЮЧЕНИИ — запись не держит никто`,
    ...
    runVerdictPartial: (names) => `Всё запущенное зелёное, но прогон урезан: не запускались ${names}. Про них это не «зелёное» — их состояние неизвестно.`,
    runVerdictEmpty: "пусто: гейтов не объявлено — прогонять было нечего. Это не «зелёное»: защиты здесь нет. Поставить первую: aqk add <имя>.",
    ...
    stagedDiffersWhy:
      "гейты смотрели рабочую копию, а в коммит уйдёт индекс. Про этот коммит прогон не сказал\n  ничего. Проверь то, что уйдёт: git stash -k, потом прогон снова — либо поставь хук через\n  фреймворк pre-commit, он неиндексированное стэшит сам.",
  ...
  baseline: {
    heading: "Обязательный минимум для работы машины",
    intro: (checked, total) =>
    ...
    // ЧЬЯ ЭТО МЕРКА — СКАЗАНО ВСЛУХ. Замер 2026-09-16 по пятнадцати чужим репозиториям
    // (requests, httpx, flask, express, gin, cobra, ripgrep, uv и другие): все получили от
    // четырёх до восьми пунктов из четырнадцати. Кресты при этом ЧЕСТНЫЕ — эквивалентов у них
    // правда нет, проверено чтением их же конфигов. Но число без этой строки читается как
    // приговор хорошему проекту, а оно про другое: про готовность отдать работу машине.
    scope: "мерка одна: можно ли отдать работу агенту. Это НЕ оценка проекта — у зрелой библиотеки без агентной обвязки большинство пунктов не подтвердится, и это не приговор ей",
    by: (b) =>
      "подтверждено: " +
