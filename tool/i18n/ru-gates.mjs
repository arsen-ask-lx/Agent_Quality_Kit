...
export const ruGates = {
  ...
  ratchet: {
    ...
    notRunnable: (slug, cmd) =>
      `Гейт «${slug}» не смог завершить проверку: ${cmd}\n` +
      `Снимать долг по несостоявшейся проверке нельзя — в реестр попадут его же сообщения\n` +
    ...
    registryHead: (slug, stamp) =>
      ...
      `#\n` +
      `# Находка, которая выпадает не каждый прогон (гонка, чужой код, сеть), вычеркнулась бы\n` +
      `# в тихий прогон и вернулась бы новой. Такую пометь «~ » в начале строки, а строкой выше\n` +
      `# напиши «# постоянная: <почему>» — по отсутствию она не вычёркивается.\n` +
      `# aqk-goal: 0\n` +
      `# aqk-deadline:\n`,
  ...
  probe: {
    shallow: "клон мелкий (git clone --depth) — истории здесь нет, и это НЕ «починок не было». Полный клон: git fetch --unshallow",
