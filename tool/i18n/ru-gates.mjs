...
export const ruGates = {
  ...
  probe: {
    ...
    unprobeable: (n) => `объявлено гейтов: ${n}, пробовать нечем ни одним: проба подставляет красный образец в каталог, который гейт сканирует, а ни одна из этих команд каталога не называет. Пригодный гейт кончается каталогом проверки — например "eslint ." или "bash checks/x.sh .".`,
