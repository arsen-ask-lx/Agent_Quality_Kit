...
//
// «совет:» / «advice:» — то, что гейт сообщает человеку при зелёном исходе (снятая осознанно
// защита). Пока метка была одна, эта строка у зелёного гейта не показывалась вовсе; договор
// между печатью в check.sh и этим выражением сторожит модульная проверка «каждую метку…».
// «пусто:» / «empty:» — гейт не нашёл ни одного файла своего типа; зелёный, но обязан это сказать.
const ADVICE_RE = /^\s*(почини|fix|совет|advice|пусто|empty)\s*:/i;
