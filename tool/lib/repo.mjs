...
function pickRecipe(rec, facts, missing) {
  ...
  // Безъязыковой родной рецепт. Ключи рецептов — языки, и это верно для линтеров, но неверно
  // для инструментов, которым язык безразличен: `gitleaks` ищет ключи в любом файле, и
  // `secrets-not-in-code` объявлен `always: true`. Без этого ключа выбор был из двух плохих —
  // продублировать команду под восемь языков (тот самый повтор, против которого у нас гейт)
  // или оставить запись без родного рецепта. Второе и было: замер на десяти чужих проектах
  // 2026-09-10 дал секреты в НЕПОКРЫТЫХ у axios и у всех четырёх python-проектов, то есть там,
  // где польза наибольшая, команды для вставки не существовало.
  //
  // Стоит ПОСЛЕ языков и ПЕРЕД переносимым: свой язык точнее, а родной сильнее переносимого —
  // но только если его есть чем выполнить.
  if (recipes.native) {
    const prog = String(recipes.native).trim().split(/\s+/)[0];
    if (runnable(recipes.native)) return recipes.native;
    if (Array.isArray(missing) && !missing.includes(prog)) missing.push(prog);
  }
