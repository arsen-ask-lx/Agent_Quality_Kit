...
${c.bold("Дальше — по порядку:")}
  ...
  1. ${c.bold("Положи образцы.")} В ${c.bold("red/")} — код, на котором проверка обязана сработать.
     В ${c.bold("green/")} — тот же код, но правильный.
     ${c.dim("Зелёный важнее: он ловит проверку, которая краснеет на исправном коде.")}
  2. ${c.bold("Напиши проверку")} в check.sh. В тексте отказа — что именно сделать.
  3. ${c.bold("Заполни gate.yml:")} намерение, триггер, доказательство отказом.
  4. ${c.bold("Прогони:")} bash tool/selfcheck/gates.sh
