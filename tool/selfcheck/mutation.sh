...
# «Нечем проверить здесь» — не то же, что «переносимого рецепта нет». Первое зависит от машины
# и лечится установкой; второе — свойство записи. Счётчики разные, чтобы итог не врал о причине.
unverified() { printf '  \033[33m~\033[0m  %s\n' "$1"; UNVERIFIED=$((UNVERIFIED+1)); }
...
for GATE in "$CAT"/*/; do
  ...
  # Запись может назвать нужную ей программу полем `requires`: по первому слову команды этого
  # не видно, обёртка вокруг готового инструмента начинается с `bash`. Без этой поправки
  # `test-not-adjusted` на машине без `checkwash` выглядел как «зелёный образец покраснел», хотя
  # покраснел он от отсутствия инструмента, а не от мутации.
  NEEDS="$(sed -n 's/^requires:[[:space:]]*\(.*\)$/\1/p' "$YML" | head -1 | tr ',' ' ')"
  LACK=""
  for PROG in $NEEDS; do
    command -v "$PROG" >/dev/null 2>&1 || LACK="${LACK:+$LACK, }$PROG"
  done
  # Два разных состояния и два разных счётчика: «переносимого рецепта нет вовсе» и «рецепт есть,
  # но нужного инструмента нет здесь». Один счётчик на оба означал бы, что итог врёт про причину
  # — ровно то, что мы не прощаем гейтам. Найдено код-ревью 2026-09-07.
  if [ -n "$LACK" ]; then
    unverified "$SLUG: НЕ ПРОВЕРЕНА здесь — нужен «$LACK»"
    continue
  fi
  if [ -z "$RECIPE" ]; then
