...
# Рабочее дерево трогать нельзя: проба живёт во временном каталоге. Если бы образец попал в
# репозиторий человека, команда осмотра стала бы командой правки — и её выключили бы в тот день.
DIRTY=$( cd "$PRBP" && git status --porcelain | wc -l | tr -d ' ' )
if [ "$DIRTY" -eq 0 ]; then
  ok "probe ничего не пишет в рабочее дерево"
else
  bad "probe оставил следы в репозитории" "$(cd "$PRBP" && git status --porcelain | head -3 | tr '\n' ' ')"
fi
rm -rf "$PRBP"
