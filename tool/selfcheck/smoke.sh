...
(
  ...
  # Рецепт принудительно ПЕРЕНОСИМЫЙ, и это не придирка к фикстуре. Проба гоняет гейт по КОПИИ
  # ПРОЕКТА, а значит впервые видит его так же, как пользователь. На этой фикстуре вскрылось:
  # родной рецепт `ruff` выдаёт НАШ ЖЕ красный образец за находку пользователя, потому что ruff
  # перешёл на многострочные блоки («--> путь» отдельной строкой), а own_samples_filter
  # построчный и вырезать блок не может. Переносимый рецепт на том же проекте молчит.
  # Это дефект каталога, а не пробы, и он чинится отдельно — а здесь нужен зелёный исходный
  # прогон, иначе проверка мерила бы тот дефект вместо разницы «слепое / прикрытое».
  sed -i 's|^  todo-without-task: .*|  todo-without-task: "bash gates/todo-without-task/check.sh ."|' .aqk.yml
...
(
  ...
  node "$CLI" init && node "$CLI" add todo-without-task
  # Рецепт принудительно переносимый — по той же причине, что в проверке №109: родной `ruff`
  # выдаёт наш же красный образец за находку пользователя, гейт красный ещё до подсадки, и
  # проба честно отвечает «судить не по чему». Здесь нужен зелёный исходный прогон, иначе
  # проверка мерила бы тот дефект каталога вместо каденции.
  sed -i 's|^  todo-without-task: .*|  todo-without-task: "bash gates/todo-without-task/check.sh ."|' .aqk.yml
) >/dev/null 2>&1
