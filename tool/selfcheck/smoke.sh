...
export AQK_LANG=ru
# Полный вывод: проверки этого файла написаны до того, как doctor стал сворачивать пройденное и
# неприменимое, и ищут строки из этих списков — в том числе ОТСУТСТВИЕ строки, которое на
# свёрнутом выводе прошло бы вхолостую. Свёртку проверяют файлы smoke/*.mjs, они флаг сбрасывают.
export AQK_VERBOSE=1
...
# ПРЕДПОЛЁТ: МОЖНО ЛИ ВООБЩЕ МЕРИТЬ.
#
# Чужой разбор 2026-09-16: этот файл, запущенный в песочнице Codex, дал 15 падений из 123.
# Настоящая причина — дочерним процессам Node запрещалось запускать git и npm, `spawnSync`
# возвращал EPERM. Проверки об этом не знали и печатали обычные кресты; человек час разбирал
# несуществующие дефекты, а «дефекты AQK 0.15.0» уехало в чужой отчёт.
#
# ИРОНИЯ, РАДИ КОТОРОЙ ЭТО НАПИСАНО. `tool/lib/execution.mjs` различает три исхода —
# clean · finding · infra_error — ровно затем, чтобы сбой инструмента не выдавался за приговор
# коду. Своей собственной оснастке мы это правило не применили.
#
# ОТКАЗ ЦЕЛИКОМ, А НЕ ЧАСТИЧНЫЙ ПРОГОН. 108 зелёных из 123 при недоступном git — это не «почти
# всё хорошо», это «мерить было нечем»; такое число вводит в заблуждение сильнее, чем молчание.
# Код возврата 3 — тем же pytest отделяет INTERNAL_ERROR от провала теста (у него провал это 1).
#
# СПРАШИВАЕМ ТОГО ЖЕ, КОГО СПРОСИТ ПРОГОН. Мало проверить git в этом шелле: ломалось именно у
# подпроцессов Node, и предполёт, смотрящий только на шелл, прошёл бы, а кресты пришли следом.
preflight() {
  WHY=""
  PT="$(mktemp -d 2>/dev/null)" || WHY="mktemp не создаёт временный каталог"
  if [ -z "$WHY" ]; then
    git init -q "$PT" >/dev/null 2>&1 || WHY="git init не отработал — без него большинство проверок меряет пустоту"
    rm -rf "$PT"
  fi
  if [ -z "$WHY" ]; then
    #
    ...
    ' 2>&1) || WHY="из подпроцесса Node не запускается то, чем мы меряем — $ERR"
  fi
  [ -z "$WHY" ] && return 0
  printf '\n  \033[33m~\033[0m  прогон невозможен: %s\n' "$WHY"
  printf '      Это НЕ «проверки упали»: мерить было нечем, и частичный прогон ничего не доказывает.\n'
  printf '      Код возврата 3 — «не смогли проверить», как INTERNAL_ERROR у pytest.\n\n'
  return 1
}

preflight || exit 3
if [ "${1:-}" = "--preflight-only" ]; then
  printf '  предполёт пройден: git и npm доступны и шеллу, и подпроцессам Node\n'
  exit 0
fi
...
# --- 112. vitals: чужой хук не выдаётся за подключённый AQK -------------------
# Разбор живой интеграции 2026-09-16: в репозитории стоял диспетчер фреймворка pre-commit, AQK
# из него не вызывался ни разу, а `vitals` печатал «хук pre-commit прописан в .git/hooks».
# Человек прочитал это как «обвязка на месте». Проверялось ИМЯ в теле хука (`/pre-commit|aqk/i`),
# а слово «pre-commit» у такого хука есть всегда. Наш собственный класс «объявлено ≠ работает».
#
# Проверяются три конца сразу, иначе починка соврёт в обратную сторону: чужой хук; наш хук,
# подключённый через `.pre-commit-config.yaml` (там в .git/hooks лежит диспетчер, а решает
# конфиг); прямой вызов в теле хука.
HKP="$(mktemp -d)"
( cd "$HKP" && git init -q . && printf '# вход
' > AGENTS.md &&
  printf 'aqk: 1
entry: [AGENTS.md]
lang: ru
...
HK_OTHER=$( cd "$HKP" && AQK_LANG=ru node "$CLI" vitals 2>&1 )
( cd "$HKP" && printf 'repos:
...
HK_OURS=$( cd "$HKP" && AQK_LANG=ru node "$CLI" vitals 2>&1 )
( cd "$HKP" && rm -f .pre-commit-config.yaml &&
  printf '#!/usr/bin/env bash
