...
for GATE in "$CAT"/*/; do
  ...
  if [ -z "$README_TEXT" ]; then
    bad "$SLUG: README.md есть, но прочитать не удалось — проверка НЕ СОСТОЯЛАСЬ"
    continue
  fi
  ...
  # ПОЧЕМУ ЗДЕСЬ НЕТ КОНВЕЙЕРА. Было `printf … | grep -qE …`, и это давало ПЛАВАЮЩИЙ отказ,
  # который мы ловили трижды и не могли объяснить: запись обвинялась в отсутствии раздела,
  # который у неё есть. Причина найдена 2026-09-10 тем самым сообщением «grep вышел с кодом
  # 141», ради которого исход и был разложен на три. 141 = 128 + 13 = SIGPIPE: `grep -q`
  # выходит на ПЕРВОМ совпадении и закрывает трубу, а `printf` в этот момент ещё пишет и
  # умирает от сигнала. При `set -o pipefail` (строка 9) статусом конвейера становится 141.
  # Воспроизведено 100 из 100, когда совпадение в НАЧАЛЕ текста, — оттого и «случайная»
  # запись: у одних раздел в начале README, у других в конце.
  # Здесь-строка конвейера не создаёт, и гонки не существует.
  grep -qE '[Гг]отовы(й аналог|й инструмент|е правил)|[Гг]отового аналога' <<< "$README_TEXT"
  case $? in
    0) : ;;
  ...
  # Тот же довод, что у поиска выше: конвейер + `grep -q` = гонка с SIGPIPE.
  grep -qE 'чего НЕ ловит|Чего НЕ ловит|чего не ловит|Чего не ловит' <<< "$README_TEXT"
  case $? in
    0) : ;;
