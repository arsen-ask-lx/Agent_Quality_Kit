...
```
        обещание                   команда                   факт
  ┌──────────────────┐     ┌──────────────────┐     ┌──────────────┐
  │    AGENTS.md     │     │     .aqk.yml     │     │ код возврата │
  │   «не коммить    │ ──▶ │  secrets-not-in- │ ──▶ │   0  или  1  │
  │     секреты»     │     │  code: bash …    │     │              │
  └──────────────────┘     └──────────────────┘     └──────────────┘
     читает человек          держит машина            знает конвейер
      и может забыть          и забыть не может        и спорить не станет
```
