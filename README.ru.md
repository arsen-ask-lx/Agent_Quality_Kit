...
пакет сам — **≈0,8 МБ**; это число сверяет машина при каждом прогоне, а не наша память.
...
**Отчёт для человека — на странице прогона.** В GitHub Actions `doctor --run` сам дописывает
сводку задания: что защищено, ловит ли защита, что упало и что поручить агенту. Прав она не
требует. Полная страница с графиками лежит в `.aqk/report.html`; чтобы скачать её из прогона,
