...
# --- уровень ---
# Язык вывода закреплён: программа печатает на языке системной локали, и на англоязычной
# машине этот разбор молча возвращал пустоту — «уровень не достигнут» на уровне AQK-3.
# Тот же класс, что и везде здесь: тишина неотличима от факта.
# С прогоном, а не без: со ступени AQK-2 уровень требует доказательства — гейт обязан
# покраснеть на своём красном образце. `doctor` без `--run` честно отвечает «не доказано», и
# итог, снятый им, показывал бы AQK-1 на репозитории с двадцатью доказанными гейтами.
LVL=$(AQK_LANG=ru node tool/program.mjs doctor --run 2>/dev/null | sed -n 's/.*Уровень: AQK-\([0-9]\).*/\1/p' | head -1)
say "уровень AQK" "${LVL:-не достигнут}" "node tool/program.mjs doctor --run"
