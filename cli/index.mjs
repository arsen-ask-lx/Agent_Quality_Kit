#!/usr/bin/env node
// temper — разложить оснастку в проект и вести общий журнал шишек.
//
...
//
//   npx github:arsen-ask-lx/temper init     разложить комплект в текущий проект
//   npx github:arsen-ask-lx/temper init --force   перезаписать уже существующие файлы
//   npx github:arsen-ask-lx/temper note "..."     записать урок в общий журнал
//   npx github:arsen-ask-lx/temper doctor   проверить, что разложено и чего не хватает
...
// .temper/. Два расходящихся свода правил — худшее, что можно сделать: через месяц они врут
...
async function cmdInit(args) {
  ...
  if (skipped.length) {
    ...
    console.log(c.dim("  перезаписать: temper init --force"));
  }
...
${c.bold("Что дальше — по порядку:")}
  ...
  2. Прочитай ${c.bold(".temper/references/project-baseline.md")} — это обязательный минимум
...
${c.dim('Обжёгся на чём-то — запиши: temper note "что случилось"')}
`);
}
...
async function cmdDoctor() {
  ...
  console.log(
    missing
      ? c.yellow(
          `\n  Не хватает: ${missing}.\n` +
            c.dim("  Файлы комплекта разложит `temper init`; .gitignore и команды в AGENTS.md — руками.\n")
        )
      : c.green("\n  Всё на месте.\n")
  );
...
function findJournal() {
  const fromEnv = process.env.TEMPER_HOME;
  const candidates = [
    fromEnv,
    join(process.env.HOME || "", "projects", "temper"),
    join(process.env.HOME || "", "temper"),
  ].filter(Boolean);
...
async function cmdNote(args) {
  ...
  if (!title) die('Нужен заголовок: temper note "что произошло"');
...
switch (cmd) {
  ...
  default:
    console.log(`
${c.bold("temper")} — оснастка для разработки с агентами

  ${c.bold("temper init")}            разложить правила и методички в текущий проект
  ${c.bold("temper init --force")}    перезаписать уже существующие файлы
  ${c.bold("temper doctor")}          проверить, что разложено и чего не хватает
  ${c.bold("temper note")} "…"        записать урок в общий журнал шишек

${c.dim("Без установки:  npx github:arsen-ask-lx/temper init")}
`);
