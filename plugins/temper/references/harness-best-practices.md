<!-- file: docs/ai/harness-best-practices.md -->
...
> (`memo-decisions.md §E/G`, `recent-2026-research.md`, `sources-building-with-agents.md`). По мере сборки
...
## 🔧 Низкоуровневые (если строишь своего агента — напр. ИИ-фича в продукте)

- [ ] **14. CodeAct** — действия агента = исполняемый код (Python/bash) в sandbox, а не дискретные JSON-инструменты. _(у нас ℹ️: это устройство кодинг-агента; «что переизобретаешь, строя своего»)_

- [ ] **7. Сериализуемые декларативные компоненты** — агент/конфиг описан данными, сохраняется и поднимается из JSON. _(у нас ✅ по духу: markdown+frontmatter+schemas)_
- [ ] **8. Actor-runtime** — типизированные сообщения + pub/sub, агенты развязаны (не зовут друг друга напрямую). _(у нас ℹ️: аналог — JSON+файлы между шагами)_
- [ ] **9. CancellationToken** — кооперативная отмена зависшего/ненужного агента. _(у нас ℹ️)_
...
## 7. Сериализуемые декларативные компоненты

- **Слой:** 🔧 низкий · **Статус у нас:** ✅ по духу — агент = markdown + frontmatter, выводы = JSON-схемы.
- **Что это.** Агент, модель-клиент, контекст, память — всё описано **данными** (Pydantic-конфиг),
  сохраняется в JSON и поднимается обратно (`dump_component`/`load_component`).
- **Зачем.** Можно версионировать, диффать, переносить целую конфигурацию агентов как данные.
- **Применимость к нам.** Для dev-среды у тебя аналог уже есть (агенты — декларативные `.md`).
  Полезно вспомнить, если будешь строить агента ВНУТРИ продукта.
- **Откуда:** autogen (`_component_config.py`, `Component[Config]`).

## 8. Actor-runtime (типизированные сообщения + pub/sub)

- **Слой:** 🔧 низкий · **Статус у нас:** ℹ️ к сведению — аналог: JSON + файлы между шагами пайплайна.
- **Что это.** Агенты не зовут друг друга напрямую — шлют типизированные сообщения в runtime
  (адресно `send_message` или в топик `publish_message`), он маршрутизирует по типу/подписке.
- **Зачем (если строишь агента).** Полная развязка: подменить/размножить агента, не трогая остальных.
- **Откуда:** autogen (`_single_threaded_agent_runtime.py`, `_base_agent.py`, `@message_handler`).

## 9. CancellationToken (кооперативная отмена)

- **Слой:** 🔧 низкий · **Статус у нас:** ℹ️ к сведению.
- **Что это.** Каждая операция агента принимает токен отмены → можно корректно погасить
  зависший или ставший ненужным шаг.
- **Откуда:** autogen (`_cancellation_token.py`).
...
## 14. CodeAct (действия агента = код)

- **Слой:** 🔧 низкий · **Статус у нас:** ℹ️ к сведению — это устройство кодинг-агента (у нас он готовый — Claude Code).
- **Что это.** Вместо набора дискретных JSON-инструментов агент **пишет исполняемый код**
  (Python/bash) как своё действие и выполняет его в **sandbox**; результат возвращается в цикл.
- **Зачем знать.** Это ядро OpenHands и многих кодинг-агентов. Если соберёшься строить своего
  кодинг-агента — переизобретать придётся именно это (CodeAct-цикл + песочница + парсинг
  действий). Ещё один аргумент использовать готового агента, а не строить.
- **Откуда:** OpenHands (`agent: CodeActAgent`; цикл в `openhands-sdk`).
