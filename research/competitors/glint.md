...
записи (`test-has-assertion` Go сейчас не покрывает вовсе), **9** — заявки на новые классы,
**37** уже закрыты готовым (golangci-lint, компилятор, сами миграторы), **77** не берём: чужая предметная
...
### 2. Новые классы — заявки в каталог, каждой нужен замер частоты — 9
...
### 3. Есть готовое — glint для этого не нужен — 37

- **сам мигратор**: `migration-duplicate-version` — golang-migrate и goose падают на дублях при
  запуске (сверено по их коду, раздел «Сверка» ниже);
...
### Сверка «берём» с популярными проверяльщиками — 2026-09-21

Вопрос владельца: не делает ли это уже кто-то. Каталоги сняты с самих инструментов, а не по
памяти: golangci-lint 2.13.2 (`help linters`), staticcheck (157 проверок, staticcheck.dev),
gocritic (108, `gocritic doc`), revive (105, `RULES_DESCRIPTIONS.md`), ruff 0.16.8 (970 правил,
`rule --all`), eslint с typescript-eslint, jest, vitest, playwright, n, unicorn и sonarjs (1353
правила из метаданных пакетов). Поиск по ключевым словам каждого класса, кандидаты прочитаны.

**Арбитр — прогон.** Один Go-модуль, где каждый класс посажен отдельной строкой. На нём
golangci-lint с `default: all` (все линтеры, gocritic и revive со всеми проверками), NilAway
(`uber-go/nilaway`, 3909 звёзд, в golangci-lint не входит) во всех режимах и glint.

| класс | кто ловит, кроме glint | glint |
|---|---|---|
| тест без утверждения, Go | **никто** (Go-каталоги пусты по этому слову) | да |
| тавтология, Go | `testifylint` `useless-assert` | да |
| `if err != nil { return nil }` | `nilerr` | да |
| `err != nil \|\| v == nil` → `nil` | `nilerr` | да |
| ошибка → `false` | никто; staticcheck S1008 советует `return err == nil`, то есть сохранить маскировку | да |
| лог уровня Error и пустое значение | никто | да |
| `return` из `main` после ошибки | никто. Рядом только staticcheck SA3000 — тот же класс, но для `TestMain` | да |
| поле из YAML никто не читает | никто | да |
| поле никто не присвоил, а читают | NilAway — **только** с `-experimental-struct-init-v2` и при вызове в коде; в режимах по умолчанию молчит. `exhaustruct` задевает случайно: ругается на любой неполный литерал | да |
| сеттер зовут не везде | никто | да |
| typed nil в интерфейсе | никто на этом образце (staticcheck SA4023 — про сравнение, другая форма) | **нет** |
| «пропустить, если нет ключа» | никто | да |
| заглушка `not implemented` | никто (`err113` ругает `errors.New` вообще) | да |
| `Mock` в прод-коде | никто | да |
| «// X удалён» | никто (ruff ERA001, sonarjs `no-commented-code` — про закомментированный КОД, не про записку) | да |
| две миграции с одним номером | **сами миграторы**: golang-migrate `ErrDuplicateMigration`, goose «found duplicate migration version» — падают при запуске | да |

**Что меняет в выводе.** Из 16 пунктов готовое есть у трёх: `testifylint`, `nilerr` (оба в
golangci-lint) и сами миграторы. `migration-duplicate-version` из заявок **снят**: беда не
молчаливая, мигратор падает громко. Остальные 12 — работа, которую никто из популярных не делал;
брать там glint, а не писать своё.

**Побочная находка про наш собственный `test-has-assertion`.** Для JS/TS то, что делает наш
`check.sh`, уже есть готовым: `expect-expect` в плагинах jest, vitest и playwright
(«Enforce assertion to be made in a test body») и `no-skipped-tests` в sonarjs («Tests should not
be skipped without providing a reason»). Для Python в 970 правилах ruff такого правила нет —
там переносимая проверка оправдана. Нарушает ли это наше правило «сперва готовое» — вопрос к
разбору записи отдельно: у `check.sh` есть тавтологии, которых `expect-expect` не видит.

### Замер: «ошибка, а код выхода 0» в TypeScript и Python — 2026-09-21

Класс пришёл из `main-return-after-error` (Go). Прежде чем заводить запись под TS и Python, где
большинство (замер языков выше), — проверка, живёт ли он там.

**Форма существует, проверено вживую** (Node 22, Python 3.12): `main().catch(console.error)` —
код 0, а голый `main()` без обработчика — код 1: «аккуратный» обработчик и прячет провал. В
Python `except Exception as e: print(e)` в блоке `__main__` — 0, с `sys.exit(1)` — 1. Готового
правила нет: `unicorn/prefer-top-level-await` красит все три варианта, включая верные, ruff
`BLE001` — и плохой, и исправленный Python.

**Частота.** 61 репозиторий (TS/JS и Python из выборки с коммитами Claude за сентябрь), мелкий
клон. Детектор: `.catch(...)` на верхнем уровне без `process.exit`/`exitCode`/`throw`; в Python
по разбору синтаксиса — широкий `except` прямо в теле `main()` или `__main__`, без выхода,
проброса и ненулевого возврата (сперва смотрел и внутрь циклов — 123 попадания, почти все —
законная ловля ошибки итерации в долгоживущем процессе). После сужения 77 попаданий; разобраны
руками все, где код выхода может читать машина:

| что | итог |
|---|---|
| 6 × `.catch(console.error)` в `scripts/` skillsmith, в т.ч. `verify-schema.ts` | форма настоящая, но скрипты ручные: ни `package.json`, ни конвейер их не зовут |
| хуки Claude Code в claude-kit (5) и impeccable (2) | **намеренно**: «a hook must never block work», сказано в коде |
| `tools/qa.py` (sg-widgets-qt) | ложное: `except` ставит флаг, выход `1 if failed` |
| `send_scheduling_stall_report.py` — по расписанию из GitHub Actions | ложное: сбой второго раздела изолирован специально, так и написано |
| `autonomy_control.py` — из обёртки `.sh` | ложное: обработчик ставит `exit_code = 2` |
| остальные ~60 | разовые ручные скрипты (`*_oneoff.py`, `_check_*.py`): ошибку видит человек в терминале |

**Вывод: запись не заводим.** Там, где код выхода читает машина, — ноль подтверждённых случаев
на 61 репозитории. Форма живая, но стоит мало. По правилу `growth/SKILL.md` идея без замера в
работу не берётся — а этот замер её не подтвердил. Второй класс плана, «настройка в конфиге ни
на что не влияет», для TS и Python не мерили: без разбора всей программы по типам его не
увидеть, а своего такого анализатора у комплекта нет и не будет (ноль зависимостей).
...
чужое читает редко. **Отправлено 2026-09-22 pull request'ом** `aiseeq/glint#4`: правка в трёх местах,
их тесты зелёные. Дефекты 1 и 4 не отправлялись.
